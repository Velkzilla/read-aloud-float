...
function getSettings(names) {
  return new Promise(function(fulfill) {
    brapi.storage.local.get(names || ["voiceName", "rate", "pitch", "volume", "showHighlighting", "languages", "highlightFontSize", "highlightWindowSize", "preferredVoices", "disableFloatingWidget"], fulfill);
  });
}
...
function clearSettings(names) {
  return new Promise(function(fulfill) {
    brapi.storage.local.remove(names || ["voiceName", "rate", "pitch", "volume", "showHighlighting", "languages", "highlightFontSize", "highlightWindowSize", "preferredVoices", "disableFloatingWidget"], fulfill);
  });
}
...
async function getSpeechVoice(voiceName, lang) {
  ...
  if (!voice && lang) {
    ...
    voice = findVoiceByLang(voices.filter(isOfflineVoice), lang)
      ...
      || await googleTranslateTtsEngine.ready().then(() => true, err => false)
